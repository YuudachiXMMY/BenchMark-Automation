...
GAME_EXECUTOR = "ffxiv-heavensward-bench.exe"
GAME_NAME = "FINAL FANTASY XIV: Heavensward Benchmark"
...
logger = utils.logger.logger("FFXIV3", dir="scripts")
...
def initialize():
    ...
    BENCH_DIRECTORY = PG.getDirectories().get("FFXIV3_Directory") + "//"
...
def start():
    ...
    try:
        ...
        try:
            ...
            logger.error('Unknown Error: FFXIV3.main()', exc_info=True)
        else:
            if statusCode == 0:
                logger.error('FFXIV3: OpenLauncherFailed', exc_info=True)
                ...
                return 0
            # try:
            #     logger.info('Killing process: FFXIV3.main()')
            #     gameHD = win32gui.FindWindow("{GAME_NAME}".format(GAME_NAME=GAME_NAME))
            #     if gameHD != 0:
            #         statC = u.killProgress("launcher.exe")
            # except Exception:
            #     logger.debug('Killing process: FFXIV3.main()')
        logger.info("Finish FFXIV3")
...
def main(pg):
    '''
    Main function for FFXIV3 automation
    '''
    global PG
    PG = pg
