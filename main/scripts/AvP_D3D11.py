...
logger = lib.logger.logger("AvP_D3D11", dir="scripts")
...
def searchLog(starting_time):
    ...
    while(c < datetime.datetime.now()):
        ...
        res = utils.searchFile("{DOCUMENT_ROOT}//{GAME_DIRECTORY}//".format(DOCUMENT_ROOT=DOCUMENT_ROOT, GAME_DIRECTORY=GAME_DIRECTORY), "AVP_%s-\d\d.txt"%(cur_time))
        if res:
            f.extend(res)
            return f
...
def startGame(loop=-1):
    ...
    while(loop!=0):
        ...
        if STRESS_TEST:
            ...
            lib.keyboardUtils.stressBenchmarking(130)
        else:
            logger.info(_TAB+'Performing Normal Test')
            lib.keyboardUtils.normBenchmarking(130)
