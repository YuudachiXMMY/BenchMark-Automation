...
GAME_EXECUTOR = "ffxiv-stormblood-bench.exe"
GAME_NAME = "FINAL FANTASY XIV: Stormblood Benchmark"
...
logger = utils.logger.logger("FFXIV4", dir="scripts")
...
def startGame():
    ...
    while(loop!=0):
        ...
        ffxiv4 = auto.PaneControl(searchDepth=1,Name='FINAL FANTASY XIV: Stormblood Benchmark')
        ffxiv4.SetTopmost(True)
        ffxiv4.ButtonControl(foundIndex=16, Name='').Click()
...
def initialize():
    ...
    BENCH_DIRECTORY = PG.getDirectories().get("FFXIV4_Directory") + "//"
...
def start():
    ...
    try:
        ...
        try:
            ...
            logger.error('Unknown Error: FFXIV4.main()', exc_info=True)
        else:
            if statusCode == 0:
                logger.error('FFXIV4: OpenLauncherFailed', exc_info=True)
                ...
                return 0
            # try:
            #     logger.info('Killing process: FFXIV4.main()')
            #     gameHD = win32gui.FindWindow("{GAME_NAME}".format(GAME_NAME=GAME_NAME))
            #     if gameHD != 0:
            #         statC = u.killProgress("launcher.exe")
            # except Exception:
            #     logger.debug('Killing process: FFXIV4.main()')
        logger.info("Finish FFXIV4")
...
def main(pg):
    '''
    Main function for FFXIV4 automation
    '''
    global PG
    PG = pg
