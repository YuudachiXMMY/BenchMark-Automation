...
logger = lib.logger.logger("keyboard", dir='lib')

## VK_CODE
VK_CODE1 = lib.VK_CODE.VK_CODE().getVK_CODE1()
VK_CODE2 = lib.VK_CODE.VK_CODE().getVK_CODE2()
...
def key_input(key, t=0.05):
    ...
    if key in VK_CODE1:
        ...
        return 1
    logger.debug("Failed to Press Key: %s"%key)
    return 0
...
def key_enter(t=0.5):
    ...
    @param:
        - t - time period in second between pressdown and pressup (default to 0.05).
    '''
    win32api.keybd_event(VK_CODE1["enter"],0,0,0)
    time.sleep(t)
    win32api.keybd_event(VK_CODE1["enter"],0,win32con.KEYEVENTF_KEYUP,0)
...
def key_space(t=0.5):
    ...
    @param:
        - t - time period in second between pressdown and pressup (default to 0.05).
    '''
    win32api.keybd_event(VK_CODE1["spacebar"],0,0,0)
    time.sleep(t)
    win32api.keybd_event(VK_CODE1["spacebar"],0,win32con.KEYEVENTF_KEYUP,0)
